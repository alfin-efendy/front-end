...
export default function AnnotationPage() {
  ...
  const [isAnnotationListCollapsed, setIsAnnotationListCollapsed] =
    useState(true);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const annotationsToSubmit = boundingBoxes.map((box) => {
        ...
        const matchingLabel = data?.labels?.find(
          (label: any) => label.name === box.label
        );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      >
                  ...
                  <div className="overflow-auto w-full flex flex-row items-center content-start space-x-3 p-2">
                    {data.labels.map((label: any) => (
                      <div
                        ...
                        className="inline-flex items-center rounded-full px-3 py-1 text-sm font-medium shadow min-w-16 cursor-grab active:cursor-grabbing hover:opacity-80 transition-opacity"
        ...
        <ResizablePanel
          defaultSize={15}
          className="border-l border-primary-foreground overflow-hidden"
        >
              ...
              {data?.tasks ? (
                <div className="space-y-3">
                  {data.tasks.map((task: any) => {
                    const isActive =
                      String(task.id) === searchParams.get("task");
                    return (
                      <div
                        key={task.id}
                        ...
                        onClick={() =>
                          handleTaskChange(String(task.id), task.urlFile)
                        }
                      >
                          <img
                            src={task.urlFile}
      ...
      <div
        className={`border-t flex flex-col flex-shrink-0 ${
          isAnnotationListCollapsed ? "h-auto" : "h-64"
        }`}
      >
        ...
        <div className="flex items-center justify-between p-3 border-b">
          <div className="flex items-center space-x-2">
            <Button
              ...
              size="sm"
              onClick={() =>
                setIsAnnotationListCollapsed(!isAnnotationListCollapsed)
              }
                  ...
                  <div
                    ...
                    className={`p-3 border rounded-lg cursor-pointer transition-colors ${
                      selectedBoxId === box.id
                        ? "border-primary bg-primary/10"
                        : "border-primary-foreground hover:border-primary/20 hover:bg-primary/5"
                    }`}
